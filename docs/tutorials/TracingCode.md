...
1. `Step into` the line where user input in parsed from a String to a Command.
    ...
    **`PatientBookParser\#parseCommand()`**
...
1. Stepping into the `switch`, we obviously stop at **`PatientBookParser\#parseCommand()`.**
...
1. We can a closer look at how storage works by repeatedly stepping into the code until we arrive at
    `JsonPatientBook#savePatientBook()`.

1. Again, it appears that the heavy lifting is delegated. Let’s take a look at `JsonSerializablePatientBook`'s constructor.

    **`JsonSerializablePatientBook\#JsonSerializablePatientBook()`:**
    ...
    * {@code JsonSerializablePatientBook}.
    */
...
1. It appears that a `JsonAdaptedPerson` is created for each `Person` and then added to the `JsonSerializablePatientBook`.
...
more about the inner workings of PatientBook and how the various
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the patient book in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Person`

    6.  Add a new entity to the patient book
